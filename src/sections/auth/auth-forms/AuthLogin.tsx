...
const AuthLogin = ({ providers, csrfToken }: any) => {
  ...
  return (
    <Formik
      ...
      const config = await fetch('/api/login', {
        method: 'POST',
        headers: {
      ...
      const response = await config.json();
          ...
          if (config.ok || response.status === 'treatmentSuccess') {
            const authToken = response.data.primaryData.authorization;
            console.log("auth token saved");
            console.log(response.data.primaryData.authorization);
