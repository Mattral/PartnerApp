...
import { VideoQuality } from '@zoom/videosdk'; // 1.12.5
...
export function getVideoLayout(rootWidth: number, rootHeight: number, count: number): CellLayout[] {
  ...
  const preferredLayout: Layout = layoutOfCount
    ...
    .reduce(
      (prev, curr) => (curr.cellArea > prev.cellArea ? curr : prev),
  ...
  // Declare cellDimensions inside the function scope
  const cellDimensions: CellLayout[] = [];
  let quality: VideoQuality = VideoQuality.Video_90P; // Default value

  if (actualCount <= 4 && cellBoxHeight >= 510) {
    quality = VideoQuality.Video_1080P; // GROUP HD
